...
#from ks3.connection import Connection as KS3Connection  #TODO
import numpy as np
...
if "KS3_ENDPOINT" in os.environ:
    KS3_HOST = os.environ["KS3_ENDPOINT"]
else:
    KS3_HOST = 'ks3-cn-beijing-internal.ksyun.com'
...
def maybe_download(filename, work_directory, source_url):
  ...
  if not gfile.Exists(filepath):
    download_ks3_file(source_url, filepath)
...
def analyze_ks3_path(ks3_path):
  if ks3_path.find('@') == -1:
    # eg. ks3://bucket/path/to//trainer-1.0.tar.gz
    m = re.match(r"(\w+)://([^/]+)/(.+)", ks3_path)
    if m:
      # return ('ks3', 'access-key-id', 'access-secret-key', 'bucket', 'path/to//trainer-1.0.tar.gz')
      return m.groups()[0], os.environ["KSC_ACCESS_KEY"], os.environ["KSC_SECRET_KEY"], m.groups()[1], m.groups()[2]
  else:
    # eg. ks3://access-key-id:access-secret-key@bucket/path/to//trainer-1.0.tar.gz
    m = re.match(r"(\w+)://(.*):(.*)@([^/]+)/(.+)", ks3_path)
    if m:
      # return ('ks3', 'access-key-id', 'access-secret-key', 'bucket', 'path/to//trainer-1.0.tar.gz')
      return m.groups()
...
def download_ks3_file(url, filename):
  # get ak,sk,bucket_name,key_name from url
  _, ak, sk, bucket_name, key_name = analyze_ks3_path(url)
  conn = KS3Connection(ak, sk, host=KS3_HOST)
